...
@section('content')
    <div class="container mt-3">
        <div class="row">
            <div class="col-6">
                <form action="{{ route('data.order.search') }}" method="GET">
                    @csrf
                    <div class="input-group">
                        ...
                        <a class="btn btn-secondary" type="submit" href="{{ route('data.order.index') }}" >Reset</a>
                    </div>
                </form>
            </div>
            <div class="col-3"></div>
            <div class="col-3">
                <div class="d-flex justify-content-end">
                    <a href="{{ route('data.order.download-excel') }}" class="btn btn-success">Export Excell</a>
                </div>
            </div>
        </div>
    </div>
        ...
        <tbody>
            @foreach ($orders as $order)
                <tr>
                    ...
                    <td><a href="{{ route('data.order.download', $order['id']) }}" class="btn btn-primary">Cetak</a></td>
                </tr>
                @endforeach
            </tbody>
    </table>
